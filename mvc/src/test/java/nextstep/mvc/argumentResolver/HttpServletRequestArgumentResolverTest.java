...
class HttpServletRequestArgumentResolverTest {
	...
	@DisplayName("HttpServletRequest를 지원하는 resolver인지 확인한다.")
	@Test
	void support_true() throws NoSuchMethodException {
		// given
		Method thisMethod = getClass().getDeclaredMethod("request", HttpServletRequest.class);

		// when
		ArgumentResolver resolver = new HttpServletRequestArgumentResolver();
	...
	@DisplayName("HttpServletRequest를 지원하지 않는 resolver인지 확인한다.")
	@Test
