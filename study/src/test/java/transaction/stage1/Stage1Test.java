...
class Stage1Test {
    ...
    void phantomReading() throws SQLException {
        ...
        final var mysql = new MySQLContainer<>(DockerImageName.parse("mysql:8.0.30"))
                ...
                // MySQL 8.0.30부터는 기본적으로 멀티 쿼리 실행이 비활성화(보안을 위해)
                // DatabasePopulatorUtils.execute() 내부 구현체를 보면 schema.sql파일을 통으로 읽어서 멀티 쿼리로 실행하고 있음
                // 멀티 쿼리를 실행하기 위해 allowMultiQueries=true 설정 필요
                // Spring Boot를 사용할 때 schema.sql을 실행하는 방식은 멀티 쿼리가 아니기 때문에, Spring Boot를 사용할 때는 허용을 해주지 않아도 됨
