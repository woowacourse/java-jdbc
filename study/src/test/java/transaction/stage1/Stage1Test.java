...
class Stage1Test {
     ...
     * + : 발생
     * - : 발생하지 않음
     * Non-Repeatable Read: 한 트랜잭션 내에서 반복 읽기를 수행하면 다른 트랜잭션의 커밋 여부에 따라 조회 결과가 달라지는 문제
    ...
    void noneRepeatable() throws SQLException {
        ...
        // 사용자B는 패스워드를 변경하고 아직 커밋하지 않았다. -> auto commit 상태이므로, 이미 커밋 된 상태!
     ...
     * phantom read: 하나의 트랜잭션에서 두번의 동일한 조회를 하였을 때, 다른 트랜잭션의 Insert로 인해 없던 데이터가 조회되는 현상
     * 격리 수준에 따라 어떤 현상이 발생하는지 테스트를 돌려 직접 눈으로 확인하고 표를 채워보자.
     * + : 발생
     * - : 발생하지 않음
