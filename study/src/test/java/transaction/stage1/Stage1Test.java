...
/**
 * 격리 레벨(Isolation Level)에 따라 여러 사용자가 동시에 db에 접근했을 때 어떤 문제가 발생하는지 확인해보자. <br/> <br/> ❗phantom reads는 docker를 실행한 상태에서
 * 테스트를 실행한다. <br/> ❗phantom reads는 MySQL로 확인한다. H2 데이터베이스에서는 발생하지 않는다. <br/>
 *
 * <p>
...
class Stage1Test {
    ...
    /**
     * phantom read는 h2에서 발생하지 않는다. mysql로 확인해보자. <br/> 격리 수준에 따라 어떤 현상이 발생하는지 테스트를 돌려 직접 눈으로 확인하고 표를 채워보자. <br/>
     * <p>
     * <strong>⭕</strong> : 발생 <br/>
     * <strong>❌</strong> : 발생하지 않음 <br/>
     * </p>
