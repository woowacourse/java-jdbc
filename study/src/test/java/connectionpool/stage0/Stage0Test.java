...
class Stage0Test {
     ...
     * JDBC 4.0 부터 Connection을 요청할 때 DriverManager가 적절한 JDBC 드라이버를 찾는다.
     * <p>
    ...
    @Test
    void driverManager_properties() throws SQLException {
        Properties properties = new Properties();
        properties.put("user", USER); // == properties.setProperty("user", USER);
        properties.put("password", PASSWORD);
        try (final Connection connection = DriverManager.getConnection(H2_URL, properties)) {
            assertThat(connection.isValid(1)).isTrue();
        }
    }
