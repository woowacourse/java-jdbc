...
class Stage1Test {
    ...
    /**
     * 커넥션 풀링(Connection Pooling)이란? <br/> DataSource 객체를 통해 미리 커넥션(Connection)을 만들어 두는 것을 의미한다. <br/> 새로운 커넥션을 생성하는 것은
     * 많은 비용이 들기에 미리 커넥션을 만들어두면 성능상 이점이 있다. <br/> 커넥션 풀링에 미리 만들어둔 커넥션은 재사용 가능하다. <br/> <br/> h2에서 제공하는
     * JdbcConnectionPool를 다뤄보며 커넥션 풀에 대한 감을 잡아보자. <br/> <br/> Connection Pooling and Statement Pooling <br/>
    ...
    /**
     * Spring Boot 2.0 부터 HikariCP를 기본 데이터 소스로 채택하고 있다. <br/>
     * https://docs.spring.io/spring-boot/docs/current/reference/htmlsingle/#data.sql.datasource.connection-pool <br/>
