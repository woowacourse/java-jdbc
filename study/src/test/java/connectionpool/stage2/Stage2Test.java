...
class Stage2Test {
    ...
    /**
     * spring boot에서 설정 파일인 application.yml를 사용하여 DataSource를 설정할 수 있다. <br/> 하지만 DataSource를 여러 개 사용하거나 세부 설정을 하려면 빈을
     * 직접 생성하는 방법을 사용한다. <br/> DataSourceConfig 클래스를 찾아서 어떻게 빈으로 직접 생성하는지 확인해보자. <br/> 그리고 아래 DataSource가 직접 생성한 빈으로 주입
     * 받았는지 getPoolName() 메서드로 확인해보자. <br/>
     */
    @Autowired
